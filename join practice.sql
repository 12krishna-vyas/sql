...
-- student who enrolled in cources
select enrollment_id , e.course_name,e.student_id from enrollments as e;

-- inner join
select s.student_id, s.student_name,e.enrollment_id,e.course_name,e.student_id from students as s join 
enrollments as e on s.student_id = e.student_id;

-- left join
select s.student_id, s.student_name,e.enrollment_id,e.course_name,e.student_id from students as s 
left join enrollments as e on s.student_id = e.student_id;

-- find the data where enrollment_id is null
-- inner joinusing where with on 
-- her student ne kitne course me enrolled kiya
-- using enrollment_id because it is unique value and not using course_name because it may be include null values
select  s.student_name,count(e.enrollment_id) from students as s 
left join enrollments as e on s.student_id = e.student_id 
group by s.student_name;

-- total cradits of each student
...
from students as s left join enrollments as e on s.student_id = e.student_id 
group by s.student_id
order by count(e.enrollment_id) desc limit 1 offset 1 ;
